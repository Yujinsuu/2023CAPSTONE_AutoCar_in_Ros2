...
class WallFollower(Node):
    ...
    def get_line_RANSAC(self, x_coords_raw, y_coords_raw) :
        ...
        y_coords = ransac.predict(x_coords)
    ...
    def scan_callback(self, data):
        ...
        # inf 값의 인덱스 찾기
        inf_indices = np.isinf(y_coords_raw)
        indices = np.where(inf_indices)[0]
        ...
        slope = np.arctan2(y_coords[-1]-y_coords[0],x_coords[-1]-x_coords[0])

        waypoints = min(len(x_coords), len(y_coords))
        wall_path = Path2D()

        for i in range(min(waypoints, 20)) :
            wx, wy, wyaw = self.change_frame(float(x_coords[i]), float(y_coords[i]), slope, self.world_frame, self.detection_frame)
